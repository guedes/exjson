defmodule ExJSON.Map.ParseTest do
  use ExUnit.Case

  test :parse_as_map do
    assert ExJSON.Map.parse("{\"key\":\"some value\"}") == %{ "key" => "some value" }
  end

  test :string_array_test do
    assert ExJSON.Map.parse('{ "another_key": [ "value1", "another value", "value 3" ] }') == %{ "another_key" => [ "value1", "another value", "value 3" ] }
  end

  test :nested_key_values do
    some_json = '{ "key": "value", "k1": "vvv", "another_key": { "k1": "v1", "k2": "v2", "k3": { "k31": 10 }, "k4": "v4" } }'

    generated_map = ExJSON.Map.parse(some_json)

    assert generated_map["another_key"]["k1"] == "v1"
    assert generated_map["another_key"]["k2"] == "v2"
    assert generated_map["another_key"]["k3"]["k31"] == 10
    assert generated_map["another_key"]["k4"] == "v4"
    assert generated_map["key"] == "value"
    assert generated_map["k1"] == "vvv"
  end

  test :complex_json do
    assert ExJSON.Map.parse(
           '{ "key": "some value",
              "another_key": [ "value1", "another value", "value 3" ],
              "nested_key": {
                "inside_key": "a value",
                "bool_value1": true,
                "bool_value2": false,
                "nil_value": null,
                "neg_value": -47.0
              },
              "tags": [ "test1", "test2", "test3" ],
              "encoded": "знач"
            }') == %{ "key" => "some value",
                      "another_key" => [  "value1", 
                                          "another value", 
                                          "value 3" ],
                      "nested_key" => %{  "inside_key" => "a value",
                                          "bool_value1" => true,
                                          "bool_value2" => false,
                                          "nil_value" => nil,
                                          "neg_value" => -47.0
                      },
                      "tags" => [ "test1", "test2", "test3" ],
                      "encoded" => "знач"
                    }
  end

end
